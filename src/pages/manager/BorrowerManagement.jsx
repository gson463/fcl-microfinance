...
const ManagerBorrowerManagement = () => {
  ...
  const handleUpdateStatus = async (newStatus) => {
    ...
    const { error } = await supabase.from('borrowers').update({ status: newStatus }).eq('id', editingBorrower.id);
  ...
  const exportBorrowersCsv = () => {
    ...
    exportObjectsToCsv(
      `borrowers_branch_${Date.now()}.csv`,
      [
  ...
  if (!branchId) {
    return (
      <DashboardLayout title="Borrowers">
        <Card>
  ...
  if (loading && borrowers.length === 0) {
    return (
      <DashboardLayout title="Branch borrowers">
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col md:flex-row justify-between gap-4">
            <CardTitle>Borrowers in your branch ({totalCount.toLocaleString()})</CardTitle>
        ...
        <DialogContent>
          ...
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="status-select-mgr" className="text-right">
                Status
              </Label>
              <Select
                key={editingBorrower?.id}
                id="status-select-mgr"
...
export default ManagerBorrowerManagement;
