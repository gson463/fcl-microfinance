-- Projected tomorrow = unpaid installments due on calendar tomorrow (CURRENT_DATE + 1 in DB timezone).
-- Reverts period-based projection from 20260509160000; drilldown UI hides date range for this metric.

-- Re-deploys get_admin_dashboard_metrics + get_admin_dashboard_drilldown + officer_projected_tomorrow_by_center
-- (same bodies as 20260509160000 with projection date logic aligned to calendar tomorrow only).
...
BEGIN
  v_projection_due := CURRENT_DATE + 1;
...
COMMENT ON FUNCTION public.officer_projected_tomorrow_by_center(uuid, uuid, date) IS
  'Loan officer: unpaid installment amounts due on calendar tomorrow (CURRENT_DATE + 1), by borrower centre. Optional p_period_end is ignored.';
