-- Optional branch + loan officer filters for admin dashboard metrics and drilldown

DROP FUNCTION IF EXISTS public.get_admin_dashboard_metrics(date, date);
DROP FUNCTION IF EXISTS public.get_admin_dashboard_drilldown(text, date, date, int, int);
...
GRANT EXECUTE ON FUNCTION public.get_admin_dashboard_metrics(date, date, uuid, uuid) TO authenticated;
