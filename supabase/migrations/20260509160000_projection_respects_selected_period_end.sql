-- "Projected tomorrow" tracks installments due the day after max(selected period end, today) — not only calendar tomorrow.
-- Fixes: date picker on dashboard/drilldown now changes which due date the projection uses.
...
schedule_agg AS (
  SELECT
    ...
    COALESCE(SUM(si.amount) FILTER (
      WHERE si.due_date = (SELECT GREATEST(COALESCE(p_end_date, CURRENT_DATE), CURRENT_DATE) + 1)
        AND si.paid_amount < si.amount - 0.01
...
BEGIN
  v_projection_due := GREATEST(COALESCE(p_end_date, CURRENT_DATE), CURRENT_DATE) + 1;
...
-- Breakdown by centre: align due date with dashboard "Projected tomorrow" (day after max(period end, today)).
...
BEGIN
  ...
  v_due := GREATEST(COALESCE(p_period_end, CURRENT_DATE), CURRENT_DATE) + 1;
...
COMMENT ON FUNCTION public.officer_projected_tomorrow_by_center(uuid, uuid, date) IS
  'Loan officer: unpaid schedule installment amounts due on (GREATEST(period_end, today)+1), grouped by borrower centre. Pass p_period_end from dashboard range end (same as get_admin_dashboard_metrics).';
