-- Add expected_today to admin dashboard metrics + drilldown (installments due today, unpaid)
...
BEGIN
  ...
  ELSIF p_metric = 'expected_today' THEN
    ...
    SELECT COALESCE(jsonb_agg(row_to_json(x)::jsonb), '[]'::jsonb) INTO v_rows FROM (
      SELECT l.id, l.loan_id, l.principal, l.balance, l.status::text AS status,
        b.first_name || ' ' || b.surname AS borrower_name, br.name AS branch_name,
