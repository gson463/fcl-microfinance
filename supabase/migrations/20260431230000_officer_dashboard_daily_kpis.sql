-- Officer dashboard daily KPIs: disbursed_today, collected_today, expected_tomorrow + drilldowns (disbursed_today, collected_today, expected_tomorrow, my_borrowers).
-- Replaces get_admin_dashboard_metrics / get_admin_dashboard_drilldown (extends 20260430120000).

-- Must drop exact signatures (CREATE OR REPLACE cannot change OUT/return type).
...
DROP FUNCTION IF EXISTS public.get_admin_dashboard_drilldown(text, date, date, integer, integer, uuid, uuid, integer);
DROP FUNCTION IF EXISTS public.get_admin_dashboard_drilldown(text, date, date, integer, integer, uuid, uuid);
